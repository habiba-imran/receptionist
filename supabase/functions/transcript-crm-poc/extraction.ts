...
function buildSystemPrompt(mode: ExtractionMode): string {
  ...
  return [
    ...
    "If the caller corrects earlier information, use the latest clearly confirmed value.",
...
function buildIncrementalSystemPrompt(): string {
  return [
    ...
    "If the new transcript corrects an existing value, replace it only when the corrected full value is clear.",
